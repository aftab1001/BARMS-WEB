...
jQuery.facebooklist = function(elem, list, complete, ajax, height, filter, newel) {
    ...
    var addItem = function(item, preadded) {
        ...
        //spn.setAttribute('innertext',title);
        var aclose = document.createElement('a');
        var image = document.createElement('img');
        var input = addHiddenInput(value);
//        var fee = document.getElementById('#ctl00_ContentPlaceHolder1_myID_feed');
//        alert(fee);

        //feed = fee;
        
        $(li).attr({
            'class': 'bit-box','value':item.attr('value')
        });
        //$(li).prepend(spn);
        $(spn).prepend(title);
        ...
        $(image).attr('src', '../images/jclose.gif');
        aclose.appendChild(image);
        li.appendChild(spn);
