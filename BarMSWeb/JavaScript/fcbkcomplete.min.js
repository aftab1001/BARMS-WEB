...
jQuery.facebooklist = function(elem, list, complete, ajax, height, filter, newel) {
    ...
    var title = item.text(); var value = (item.attr('rel') && item.attr('rel') != -1 ? item.attr('rel') : title); var li = document.createElement('li'); var txt = document.createTextNode(title); var spn = document.createElement('span');  var aclose = document.createElement('a'); var input = addHiddenInput(value); $(li).attr({ 'class': 'bit-box','value':item.attr('value') });
    $(spn).prepend(title);
    $(aclose).attr({ 'class': 'closebutton', 'href': '#' }); li.appendChild(spn); li.appendChild(aclose); li.appendChild(input); holder.appendChild(li); $(aclose).click(function() { $(this).parent('li').fadeOut('fast', function() { $(this).remove(); }); return false; }); if (!preadded) { holder.removeChild(document.getElementById('annoninput')); addInput(); }
        feed.hide();
    }
    var defaultFilter = function(input) {
        ...
        if (counter > height) { feed.css({ 'height': (height * 24) + 'px', 'overflow': 'auto' , 'z-index':'2' }); }
        else { feed.css('height', 'auto'); } 
    }
    var feedFilter = function(item, caption, input) {
        if (filter.userfilter) {
            if (filter.casesensetive) { if (caption.indexOf(input) != -1) { item.html(caption.replace(input, '<em>' + input + '</em>')); return true; } }
    ...
    var addInput = function() {
        var li = document.createElement('li'); var input = document.createElement('input'); $(li).attr({ 'class': 'bit-input', 'id': 'annoninput' }); $(input).attr({ 'type': 'text', 'class': 'maininput' }); li.appendChild(input); holder.appendChild(li); $(input).focus(function() { complete.fadeIn('fast'); }); $(holder).click(function() {
            $(input).focus(); if (feed.length && $(input).val().length) { feed.show(); }
            else { feed.children('li[fckb=2]').remove(); feed.children('li').addClass('hidden'); feed.css('height', '0px'); $('.default').show(); } 
        }); $(input).keyup(function(event) {
            if (event.keyCode != 40 && event.keyCode != 38) {
                counter = 0; var etext = $(input).val(); addTextItemFeed(etext); if (ajax.url) {
                    if (ajax.cache && cache.length > 0) { addItemFeed(cache, etext); bindEvents(); }
